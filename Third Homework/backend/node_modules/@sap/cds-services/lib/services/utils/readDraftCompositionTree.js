...
const _containsAllBackLinks = (backLinks, subRows, rows) => {
  return backLinks.every(backlink => subRows[backlink.target_element] === rows[backlink.name])
}
...
const readDraftCompositionTree = async (service, context) => {
  ...
  const compSet = getCompositionSet(service.model.definitions, ensureNoDraftsSuffix(context.target.name))
