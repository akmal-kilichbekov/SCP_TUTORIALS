const {
  BatchExitHandler: { ATOMICITY_GROUP_START, ATOMICITY_GROUP_END, BATCH_START, BATCH_END },
  Components: {
    DATA_CREATE_HANDLER,
    DATA_DELETE_HANDLER,
    DATA_READ_HANDLER,
...
const { end, start } = require('./handlers/batch')
...
class OData {
  ...
  _createOdataService (edm) {
    ...
    this._odataService = ServiceFactory.createService(
      edm,
      require('./utils/oDataConfiguration')(edm, this._options)
    ).trust(process.env.NODE_ENV === 'production')
  ...
  addCDSServiceToChannel (cdsService) {
    ...
    this._odataService.on(BATCH_START, start)
