 ...
 * @private
 */
module.exports = (modification, target, user) => {
  ...
  for (const column of columns) {
    if (column[cdsAnnotation]) {
      column.value = column[cdsAnnotation]['='] === '$user' ? user.id : new Date().toISOString()
    } else {
      column.value = column[odataAnnotation]['#'] === 'user' ? user.id : new Date().toISOString()
    }
  }
