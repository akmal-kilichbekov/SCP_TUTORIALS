...
class FsHelper {
    ...
    static _addTouchedFile(filepath) {
    ...
    static writeJSON(filepath, object, options = {spaces: 4}, skipFileLog = false) {
        if (!skipFileLog) {
            this._addTouchedFile(filepath);
        }
    ...
    static async copyTemplate(template, destinationPath, templateValues, overwrite = false) {
    ...
    static async _processTemplateFile(src, destination, templateValues, overwrite = false) {
    ...
    static getTouchedFiles() {
        return Array.from(_touchedFiles.keys());
    }


    static mkdirp(folderPath) {
