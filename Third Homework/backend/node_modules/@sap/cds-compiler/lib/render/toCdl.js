...
function toCdsSource(model, options) {
  ...
  if(plainNames) // enable duplicates check only for plain mode
    gRendreg =  require("./rendreg")(model); // registry for all artifact names and element names
  ...
  for (let artifactName in getTopLevelArtifacts()) {
    ...
    let env = createEnv();
    let sourceStr = renderArtifact(artifactName, csn.definitions[artifactName], env, model.definitions[artifactName]); // Must come first because it populates 'env.topLevelAliases'
    if (sourceStr != '') {
  ...
  gRendreg && gRendreg.done(); // perform duplicates check
  ...
  function renderArtifact(artifactName, art, env, xart) {
    ...
    switch (art.kind) {
      ...
      case 'view':
        ...
        if (art.query) {
          ...
          return renderEntity(artifactName, art, env, xart);
        }
      case 'context':
      case 'service':
...
*/
  ...
  function renderEntity(artifactName, art, env, xart) {
    let result = renderAnnotationAssignments(art, env);
    let childEnv = increaseIndent(env);
    let normalizedArtifactName = renderArtifactName(artifactName, env);
    gRendreg && gRendreg.addArtifact(normalizedArtifactName, xart && xart.name.location);
    ...
    rendreg.addArtifact(artifactName, xart && xart.name.location)
    for (let name in art.elements) {
      result += renderElement(name, art.elements[name], childEnv, rendreg, xart && xart.elements[name]);
    }
    rendreg.done();
    result += env.indent + '}';
  ...
  // Return the resulting source string.
  function renderElement(elementName, elm, env, rendreg, xart) {
    ...
    rendreg && xart && rendreg.addElement(quoteOrUppercaseId(elementName), xart.name.location);
