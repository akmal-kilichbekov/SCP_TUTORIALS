...
function toSqlDdl(model) {
  ...
  let rendreg = require("./rendreg")(model); // registry for all artifact names and element names
  ...
  for (let artifactName in csn.definitions) {
    ...
    renderArtifactInto(artifactName, csn.definitions[artifactName], resultObj, env, model.definitions[artifactName]);
  }

  rendreg.done(); // trigger artifact and element name checks
  ...
  function renderArtifactInto(artifactName, art, resultObj, env, xart) {
    ...
    switch (art.kind) {
      ...
      case 'view':
        if (art.query) {
          let result = renderView(artifactName, art, env, xart);
          if (result) {
            resultObj.hdbview[artifactName] = result;
          }
        } else {
          renderEntityInto(artifactName, art, resultObj, env, xart);
        }
        break;
      case 'type': {
        let result = renderType(artifactName, art, env, xart);
        if (result) {
          resultObj.hdbview[artifactName] = result;
        }
        break;
  ...
  function renderEntityInto(artifactName, art, resultObj, env, xart) {
    ...
    rendreg.addArtifact(tableName, xart.name.location)
    result += 'TABLE ' + tableName;
    result += ' (\n';
    let elements = Object.keys(art.elements).map(name => renderElement(artifactName, name, art.elements[name], getFzIndex(name, hanaTc), childEnv, xart.elements[name]))
                                       .filter(s => s != '')
                                       .join(',\n');
    if (elements != '') {
      result += elements;
  ...
  function renderElement(artifactName, elementName, elm, fzindex, env, xart) {
    ...
    rendreg.addElement(quotedElementName, xart.name.location);
  ...
  // Render a view
  function renderView(artifactName, art, env, xart) {
    env._artifact = art;
    let viewName = quoteSqlId(absoluteCdsName(artifactName));
    rendreg.addArtifact(viewName, xart.name.location)
  ...
  // Render a type.
  // Return the resulting source string.
  function renderType(artifactName, art, env, xart) {
    ...
    rendreg.addArtifact(typeName, xart.name.location)
    let result = 'TYPE ' + quoteSqlId(absoluteCdsName(artifactName)) + ' AS TABLE (\n';
    let childEnv = increaseIndent(env);
    if (art.elements) {
      // Structured type
      let elements = Object.keys(art.elements).map(name => renderElement(artifactName, name, art.elements[name], null, childEnv, xart.elements[name]))
                                         .filter(s => s != '')
                                         .join(',\n') + '\n';
      if (elements != '') {
        result += elements;
