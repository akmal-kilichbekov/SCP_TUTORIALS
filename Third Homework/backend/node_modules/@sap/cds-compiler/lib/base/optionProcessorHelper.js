...
function createOptionProcessor() {
  ...
  function verifyOptions(options, command = undefined) {
    ...
    if (options && options.assocOnRewrite && !options.newRedirectImpl) {
      throw new Error(`Option "--assoc-on-rewrite" can only be used with "--new-redirect-impl"`);
    }
    if (options && options.newAutoExpose && !options.newRedirectImpl) {
      throw new Error(`Option "--assoc-on-rewrite" can only be used with "--new-redirect-impl"`);
    }
