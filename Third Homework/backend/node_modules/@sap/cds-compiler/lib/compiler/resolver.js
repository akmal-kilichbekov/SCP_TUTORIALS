...
const { getMessageFunction, hasErrors, searchName, weakLocation } = require('../base/messages');
const { dsome } = require('../transform/udict.js');
const { getDefinerFunctions, augmentPath, splitIntoPath } = require('./definer');
...
function resolve( model ) {
  ...
  const { initArtifacts, hasTruthyProp } = getDefinerFunctions(model);
  let newAutoExposed = [];
  return main();

  function main() {
    if (!options.tntFlavor || options.tntFlavor.skipLateAutoExposure)
      options.newRedirectImpl = true;
    ...
    if (options.assocOnRewrite || options.newRedirectImpl) {
    ...
    // Phase 7: create convenience views
    createLocalizedConvenienceViews(model);
    // Phase 8: apply ANNOTATE on unknown artifacts:
    if (model.extensions)
  ...
  function finalType( art ) {
    ...
    if (art && art.target && options.newRedirectImpl)
  ...
  /**
   * Main function to create all localized convenience views (for entities and views).
   * @param model - The XSN model
   */
  function createLocalizedConvenienceViews(model) {
    // skip ceation if model contains any errors
    if (hasErrors(model.messages)) {
      return;
    }
    // cache the information for each entity if it is localized or not
    let localizedCache = {};

    // step 1: create localized convenience views for entities
    let convenienceViews = {}; // cache convenience views for entities
    // step 1.1: create basic convenience view artifact
    forEachDefinition( model, prepareLocalizedConvenienceView(convenienceViews, localizedCache) );
    // step 1.2: enrich convenience view artifact (rewrite assoc and init artifact)
    for (let viewName in convenienceViews) {
      let info = convenienceViews[viewName];
      initializeLocalizedConvenienceView(viewName, info.view, info.base, convenienceViews);
    }
    // step 1.3: execute compile phases that have already been done for other artifacts on conv views
    for (let viewName in convenienceViews) {
      let view = convenienceViews[viewName].view;
      // execute sub phases which have already been executed in general:
      populateView( view )
      propagateKeyProps ( view );
      resolveRefs( view );
    }
    // step 2.1: create localized convenience views for views
    let views = initializeLocalizedConvenienceViewsForViews(model._entities, localizedCache);
    // step 2.2: rewrite _artifact and $navigation properties
    for (let view of views) {
      rewriteToLocalized(view);
    }
  }

  /**
   * Returns if an artifact has a localized element or an association that leads
   * directly or indirectly to another artifact that has a localized element.
   * @param art - the main artifact (entity or view) to be checked
   * @param localizedCache - a map used as cache with absolute artifact name as key
   */
  function isLocalizedOrAssocToLocalized(art, localizedCache) {
    let reached = {};
    return recurse(art);

    function recurse(art) {
      // add entry to avoid circular paths
      reached[art.name.absolute] = true;
      // if element is in cache
      if (localizedCache[art.name.absolute] != undefined) {
        return finalResult(localizedCache[art.name.absolute]);
      }
      // check if it has localized elements
      if (art.$localizedElements && art.$localizedElements.length) {
        return finalResult(true);
      }
      // check each association
      let localizedUnknown = false;
      for (let elemName in art.elements) {
        let element = art.elements[elemName];
        if (element.target && element.target._artifact) {
          if (reached[element.target._artifact.name.absolute]) {
            // we have an assoc to an entity where we can't evaluate in this step
            // if it is localized or not (circular)
            localizedUnknown = true;
          } else if (recurse(element.target._artifact)) {
            return finalResult(true);
          }
        }
      }
      return localizedUnknown ? false : finalResult(false);

      function finalResult(isLocalized) {
        // save information to cache and return result
        localizedCache[art.name.absolute] = isLocalized;
        return isLocalized;
      }
    }
  }
  ...
  /**
   * Returns a function which creates a localized convenience view
   * for a given entity (not view!) `art` and stores it in `results`.
   * The newly created artifact isn't initialized by this routine!
   * @param results - map containing the created localized convenience view artifacts
   * @param localizedCache - a cache used by `isLocalizedOrAssocToLocalized`
   */
  function prepareLocalizedConvenienceView(results, localizedCache) {
    return function(art) {
      let viewName = 'localized.' + art.name.absolute;
      // create localized variant for each entity
      if (art.kind === 'entity' && art.elements && (!art.abstract || !art.abstract.val) && !art.query && isLocalizedOrAssocToLocalized(art, localizedCache)) {
        ...
        let view = createLocalizedDataView( art, viewName, art.$localizedElements || [], results );
        results[viewName] = { view, base: art };
      }
    }
  }

  /**
   * Creates the localized convenience view for a given entity (not view!)
   * @returns the created convenience view
   * @param base - the original artifact
   * @param absolute - the absolute name of the localized convenience view
   * @param textElems - list of localized elements of the original artifact
   */
  function createLocalizedDataView( base, absolute, textElems ) {
    ...
    for (let localizedElem of textElems) {
      let origElement = localizedElem.elem;
      let assoc = localizedElem.assoc;
      let assocName = (assoc === 'default') ? 'localized' : assoc.name.id;
      if (!origElement.key || !origElement.key.val) {
        let location = origElement.name.location;
        let id = origElement.name.id;               // element name in the texts entity
        let newId = localizedElem.newNameId || id;  // element name in the view / projection
        let value = {           // TODO: special to allow different code in HANA?
          op: { location, val: 'call' },
          func: augmentPath( location, 'coalesce' ),
          args: [
            augmentPath( location, 'L', assocName, id ),
            augmentPath( location, 'L', newId ) ],
          location
        };
        setProp( value, '_artifact', origElement );
        let origin = {};
        setProp( origin, '_artifact', origElement );
        // TODO: stay automatically silent if "shadowed" source element appears in expression
        columns.push( { name: { id: newId, location }, location: origElement.location, value, origin, $replacement: 'silent' } );
      }
    }
    return art;
  }
  ...
  /**
   * Initializes the localized convenience views created by `createLocalizedDataView`.
   * Also redirects associations to their localized convenience view if existent.
   * @param viewName - the name of the localized convenience view
   * @param view - the localized convenience view
   * @param base - the original artifact
   * @param convenienceViews - list of localized convenience views (needed for redirection checks)
   */
  function initializeLocalizedConvenienceView(viewName, view, base, convenienceViews) {
    let location = base.location;
    // rewrite associations
    for (let elemName in base.elements) {
      let origElem = base.elements[elemName];
      // skip any non-association
      if (!origElem.target || !origElem.target._artifact) continue;
      let localizedName = 'localized.' + origElem.target._artifact.name.absolute;
      // skip if no localized target exists
      if (!convenienceViews[localizedName]) continue;
      // rewrite association to localized target
      let value = { path: [{ id: origElem.name.element, location }], location };
      let targetPath = [{ id: localizedName, location }];
      view.query.columns.push({
        name: origElem.name,
        location: origElem.location,
        redirected: { val: true, location },
        target: { path: targetPath, location },
        value,
        $replacement: 'silent'
      });
    }
    ...
    artifacts[ viewName ] = view;
    initArtifacts( { artifacts }, null, model.$internal, false, '' );
  }

  /**
   * Creates and initialized localized convenience views for views.
   * These convenience views are a simple copy of the original view with a new name ('localized.' + originalName).
   * There isn't any redirection for them to their corresponding localized artifacts as this has to happen
   * in a later step after all convenience views have been created.
   * @param entities - list of all entities of the model
   * @param localizedCache - cache used by `isLocalizedOrAssocToLocalized`
   */
  function initializeLocalizedConvenienceViewsForViews( entities, localizedCache ) {
    // skip views that aren't localized
    let localizedEntities = entities.filter((art) => {
      // skip elements that are
      // - already localized ($localizedElements or name starts with 'localized.')
      // - not a view or entity (e.g. types)
      // - abstract
      if (art.$localizedElements || art.name.absolute.startsWith('localized.') || !['view', 'entity'].includes(art.kind) || !art.elements || art.abstract) {
        return false;
      }
      ...
      // check if the view has any localized element in its projection
      let isLocalized = dsome(art.elements, (elemName) => {
        let elem = art.elements[elemName];
        if (elem instanceof Array) {
          return false; // skip duplicate definitions
        }
        return hasTruthyProp( elem, 'localized' ) || (elem.target && isLocalizedOrAssocToLocalized(elem.target._artifact, localizedCache));
      });
      return isLocalized;
    });
    // create localized convenience view
    return localizedEntities.map((art) => {
      let viewName = 'localized.' + art.name.absolute;
      // create convenience view redirecting all froms to localized world if possible
      let location = art.name.location;
      // - copy query
      let query = copyExpr(art.query, location, true, true);
      // - create view
      let localizedView = {
        kind: 'entity',
        name: { location, path: splitIntoPath( location, viewName ) },
        location: art.location,
        query,
        $inferred: 'localized'
      };
      addBoolAnnotationTo('@odata.draft.enabled', false, localizedView);

      let artifacts = Object.create(null);
      artifacts[ viewName ] = localizedView;

      // TODO: support for name.space::Base ?
      initArtifacts( { artifacts }, null, model.$internal, false, '' );
      populateView( localizedView )
      propagateKeyProps ( localizedView );
      resolveRefs( localizedView );
      return localizedView;
    });
  }

  /**
   * Modifies the given artifact (or property) `art` by replacing all _artifact and
   * _navigation links to their localized viarants if existent.
   * @param art - artifact to be rewritten to link to localized elements if possible
   */
  function rewriteToLocalized(art) {
    if (!art || typeof art !== 'object') {
      return;
    } else if (art instanceof Array) {
      art.map(e => rewriteToLocalized(e));
      return;
    }

    let proto = Object.getPrototypeOf( art );
    if (proto && proto !== Object.prototype) // do not copy object from special classes
      return;
    for (let prop of Object.getOwnPropertyNames(art)) {
      if (prop === '_artifact' || prop === '_navigation') {
        let linkedArt = art[prop];
        if (!linkedArt) {
          continue;
        }
        let localizedArt = getLocalizedVariant(linkedArt);
        if (localizedArt) {
          let descr = Object.getOwnPropertyDescriptor(art, prop);
          descr.value = localizedArt;
          Object.defineProperty(art, prop, descr);
        }
      } else {
        let descr = Object.getOwnPropertyDescriptor(art, prop);
        if (descr.enumerable) {
          rewriteToLocalized(art[prop]);
        }
      }
    }
  }
  ...
  /**
   * Returns the localized artifact variant for a given artifact if existent.
   * The artifact can also be a query element, a table alias etc., it doesn't
   * have to be a top level artifact.
   * @param art - original artifact
   */
  function getLocalizedVariant(art) {
    // get localized main artifact
    let mainArtName = art.name.absolute;
    if (mainArtName.startsWith('localized.')) return null;
    let localizedName = 'localized.' + mainArtName;
    let localizedArt = model.definitions[localizedName];
    if (localizedArt) { // if localized main artifact exists
      // Find sub artifact that matches to original artifact `art`
      // by following the description of art.name
      if (art.name.query != null) {
        localizedArt = localizedArt.$queries[art.name.query];
      }
      if (art.name.alias) {
        localizedArt = localizedArt.$tableAliases[art.name.alias];
      }
      if (art.name.element) {
        // element name with '.' in case of foreign keys or struct types
        // (localized enums don't exist)
        let elemNames = art.name.element.split('.');
        for (let elemName of elemNames) {
          // depending on the type of the sub artifact we have to take a look at different collections.
          let collection = localizedArt.elements || localizedArt.$navigation || localizedArt.foreignKeys;
          // In case of redirected associations we have to follow them as they don't have elements nor $navigation property
          if (!collection) {
            let target = getLocalizedVariant(localizedArt.target._artifact) || localizedArt.target._artifact;
            collection = target.elements;
          }
          localizedArt = collection[elemName];
        }
      }
      // This internal error triggers if we found a localized variant for the main artifact but the algorithm above
      // wasn't able to follow the path to get to the sub artifact.
      if (!localizedArt) throw Error("Unexpected error when rewriting localized convenience view");
    }
    return localizedArt;
  }
  ...
  function populateQuery( query ) {
    ...
    function initElem( elem ) {
      ...
      // console.log(message( null, elem.location, elem, {art:query}, 'Info','RED').toString(), elem.value)
  ...
  function redirectImplicitly( elem, assoc ) {
    ...
    if (service && target._service !== service) {
      ...
      let inferredAssoc = (assoc._main||assoc)._service !== service;
      let exposed = minimalExposure( target._descendants && target._descendants[ service.name.absolute ] );
      // console.log(message( null, elem.location, elem, {art: service, target}, 'Info','BLUE').toString(), exposed.length)

      if (!exposed.length) {
        let origTarget = target;
        if (isAutoExposed( target ))
          target = createAutoExposed( origTarget, service );
        let desc = origTarget._descendants ||
                   setLink( origTarget, Object.create(null), '_descendants' );
        desc[ service.name.absolute ] = [target];
      }
      else if (exposed.length === 1) {
        let autoexposed = exposed[0]['@cds.autoexposed'];
        // console.log(message( null, elem.location, elem, {art: service, target: exposed[0]}, 'Info','TO').toString(), autoexposed, exposed[0])
  ...
  function isAutoExposed( target ) {
    if (!options.newAutoExpose && !options.newRedirectImpl) // arg, there are non-compiler tests which use new-redirect
      return false;
    if (target.$autoexpose !== undefined)
      return target.$autoexpose;
    let chain = [];
    let source = target.$from && target.$from[0] && target.$from[0]._artifact;
    while (setAutoExposed( target, true ) && source) { // stop at first ancestor with annotation or at non-query entity
      chain.push( target );
      target = source;
      source = target.$from && target.$from[0] && target.$from[0]._artifact;
    }
  ...
  function createAutoExposed( target, service ) {
    let targetName = target.name.absolute;
    let absolute   = service.name.absolute + '.'
                   + ( options.longAutoexposed
                       ? targetName.replace( /\./g, '_' )
                       : targetName.substring( targetName.lastIndexOf('.') + 1 ) );
    let autoexposed = model.definitions[ absolute ];
    if (autoexposed) {
      ...
      if (autoexposed.$inferred !== 'autoexposed')
        return target;
      let firstTarget = autoexposed.query.from[0]._artifact;
      message( 'duplicate-autoexposed', service.name.location, service,
    ...
    let location = target.name.location;
    let artifacts = Object.create(null);
    let from = augmentPath( location, target.name.absolute );
    let art = {
      kind: 'entity',
      name: { location, path: splitIntoPath( location, absolute ) },
  ...
  function resolveRefs( art ) {
    ...
    if (art.kind === 'element') {
      ...
      if (!art['@Core.Computed'] && (
          ...
          (art.value && (art.value.op || art.value.literal || art.value.scope == 'param' || (art.value._artifact && art.value._artifact.kind == 'builtin')))
         ))
        art['@Core.Computed'] = {
          name: {
  ...
  function extensionFor( art ) {
    ...
    if (!art._main) {
      ...
      let kind = kindProperties[ art.kind ].normalized || art.kind; // enums would be first in elements
  ...
  function excludeAssociation( elem ) {
    let target = elem.target && elem.target._artifact;
    ...
    if (!elem.$inferred) {      // && !elem.target.$inferred
      ...
      message( 'assoc-is-autoexcluded', elem.target.location, elem,
               { target }, 'Info',
               'Association is auto-excluded, as target $(TARGET) is outside any service' );
      elem._ignore = true;      // XSN TODO: $ignore
    }
  }
  ...
  function rewriteKeys( elem, assoc ) {
    ...
    forEachInOrder( assoc, 'foreignKeys', function( orig, name ) {
      ...
      if (elem._redirected) {
        ...
        let state = rewriteItem( elem, i, i.id, elem, true );
    ...
    if (!elem) {
      if (location)
        message( 'rewrite-not-projected', location, assoc,
                 { name: assoc.name.id, art: item._artifact }, options.betaMode ? 'Error' : 'Warning', {
...
function copyExpr( expr, location, skipUnderscored, rewritePath ) {
  ...
  for (let prop of Object.getOwnPropertyNames( expr )) {
    ...
    else if ((prop.charAt() !== '$' && prop !== 'annotationAssignments') || prop === '$inferred') // TODO: rename to $annotationAssignments
      r[prop] = copyExpr( pd.value, location, skipUnderscored, rewritePath );
