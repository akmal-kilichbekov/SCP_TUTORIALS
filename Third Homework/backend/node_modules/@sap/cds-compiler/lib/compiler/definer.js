...
function getDefinerFunctions( model ) {
  ...
  const { resolveUncheckedPath, resolvePath, defineAnnotations, setAutoExposed }
        = fns( model );
  var extensionsDict = Object.create(null);
  var lateExtensionsDict = Object.create(null); // for generated artifacts
  return { define, initArtifacts, getOriginRecursive, hasTruthyProp };
  ...
  function setProjectionAncestors( art ) {
    ...
    let chain = [];
    let autoexposed = art['@cds.autoexposed'];
  ...
  function extendArtifact( extensions, art, noIncludes ) {
    if (!noIncludes && !(canApplyIncludes( art ) && extensions.every( canApplyIncludes )))
      return false;
    if (art.$entity && options.testMode)
      throw new Error('Unexpected double-extension');
    if (!art.query) {
  ...
  function extendMembers( extensions, art, noExtend ) {
    ...
    for (let ext of extensions) {
      if (!('_artifact' in ext.name)) { // not already applied
        ...
        art._deps.push( { art: ext } );      // art depends silently on ext (inverse to normal dependency!)
      }
  ...
  function applyIncludes( ext, art ) {
    ...
    for (let ref of ext.includes) {
      let template = ref._artifact; // already resolved
      if (template) {
    ...
    for (let ref of ext.includes) {
      ...
      if (template) {           // be robust
        forEach( template, prop, function( origin, name ) {
          ...
          let elem = linkToOrigin( origin, name, parent, prop, weakLocation( ref.location ) );
  ...
  function processLocalizedData( art ) {
    ...
    art.$localizedElements = (localized || []).map(elem => {
      return { elem, assoc: 'default' };
    });
  ...
  function hasTruthyProp( art, prop ) {
    let artWithProp = getOriginRecursive(art, (artToCheck) => artToCheck[prop]);
    if (!artWithProp)
      return null;
    let val = artWithProp[prop].val;
...
module.exports = { define: model => getDefinerFunctions( model ).define(), getDefinerFunctions, augmentPath, splitIntoPath };
