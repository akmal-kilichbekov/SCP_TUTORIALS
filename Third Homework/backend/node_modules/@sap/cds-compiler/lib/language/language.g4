    ...
    |
      ...
      ( FROM
        ...
        |
          ...
          {
            if (!this.options.tntFlavor || this.options.tntFlavor.skipUsingFromIgnoring) {
              // make it look exactly like the normal ANTLR syntax message
              this.message( 'syntax-mismatched-token', $id,
                            { offending: 'Identifier', expecting: 'String' },
...
annotationAssignment_tntextra[ annos ] locals[ assignment = { name: {} } ]
@after { $annos.push( this.attachLocation($assignment) ); }
  :
    simplePath[ $assignment.name, 'anno' ]
    ( '#' variant=ident['variant'] { $assignment.name.variant = $variant.id; } )?
    (
      t=':' val=value
      {
        ...
        if (!this.options.tntFlavor || this.options.tntFlavor.skipSloppyAnnoAssignments) {
          this.message( 'syntax-anno-value',
                        this.combinedLocation( $assignment.name, $val.val ),
                        { code: '@(...)' },
                        'Error', 'Use $(CODE) for annotation assignments here' );
        }
      }
    )?
  ;
    ...
    |
      ...
      ( '#' variant=ident['variant']
        { $assignment.name.variant = $variant.id;
          $assignment.name.location.end = $variant.id.location.end; } )?
      {
...
@after { /* #ATN 2 */ this.attachLocation($art); }
  :
    ...
    { $art = this.addDef( $outer, 'artifacts', $service ? 'service' : 'context', $name, $annos,
                          { abstract: $abs }, $loc ); }
    // #ATN: @ can also follow loop (2x)
    ( '@' annotationAssignment_paren[ $annos ] )*
    (
      '@' annotationAssignment_tntextra[ $annos ]
      ( '@'
        ( annotationAssignment_paren[ $annos ]
        | annotationAssignment_tntextra[ $annos ]
        )
...
@after { this.attachLocation($art); }
  :
    ...
    key=KEY?
    simplePath[ $item, 'ref' ]
    ( AS aident=ident['Item'] { $alias=$aident.id } )?
    { $art = this.addItem( $outer, 'columns', null,
                           $annos, { value: $item, name: $alias, key: $key }); }
    annotationAssignment_fix[ $annos ]*
    ( ':' re=REDIRECTED to=TO
...
@after { this.attachLocation($art); }
  :
    expected=PROJECTION simplePath[ $name, 'Entity' ]
    {
      $art = this.addItem( $outer, 'extensions', 'extend', $annos,
...
@after { this.attachLocation($art); }
  :
    ( TYPE | db=TABLE TYPE | ASPECT | event=EVENT ) simplePath[ $name, 'Type' ]
    { this.betaModeOnly( $event );
...
@after { this.attachLocation($art); }
  :
    CONST simplePath[ $name, 'Constant' ]
    { $art = this.addItem( $outer, 'extensions', 'extend', $annos,
...
@after{ /* #ATN 3 */ }
  ...
  |
    ...
    |
      ...
      | typeTypeOf[ $art.items ] annotationAssignment_ll1[ $annos ]* requiredSemi
      | typeRefOptArgs[ $art.items ]
        annotationAssignment_ll1[ $annos ]*
        ( ENUM '{' enumSymbolDef[ $art.items ]+ '}' optionalSemi | requiredSemi )
      )
    |
      typeTypeOf[ $art ] annotationAssignment_ll1[ $annos ]* requiredSemi
    |
      l=LOCALIZED { $art.localized = this.tokenLocation( $l, undefined, true ); }
      typeRefOptArgs[ $art ]
      annotationAssignment_ll1[ $annos ]*
...
@after{ /* #ATN 1 */ }
  :
    ...
    |
      a=ALL
      { this.hanaFlavorOnly( 'ALL in functions is not supported yet', $a ); }
      e1=expression { $pathStep.args = [ $e1.expr ]; }
    |
      d=DISTINCT
      { this.hanaFlavorOnly ( 'DISTINCT in functions is not supported yet', $d );}
      e1=expression { $pathStep.args = [ $e1.expr ]; }
      ( ',' e2=expression { $pathStep.args.push( $e2.expr ); } )*
    |
      star='*'
...
optionalCardinality[ pathStep ]
@after { if ($pathStep.cardinality) this.attachLocation($pathStep.cardinality); }
  :
