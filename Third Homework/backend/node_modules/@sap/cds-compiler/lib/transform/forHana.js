...
function transformForHana(inputModel, options) {
  ...
  forEachDefinition(model, a => {
    ...
    if(!isNoDbArtifact(a) && a.$queries && a.$queries.length > 0) {
      let env = {
        aliasCount: 0,
  ...
  forEachDefinition(model, (artifact, name) => {
    ...
    if (!options.forHana.keepNamespaces && artifact.kind == 'namespace' &&
        ((name !== 'cds' || model.definitions['cds.foundation']) && (name !== 'localized' || (artifact.artifacts && Object.keys(artifact.artifacts).length)))) {
      artifact.kind = 'context';
    }
    if (!artifact._ignore) {
      ...
      forEachMemberRecursively(artifact, (member) => {
        if (!member._ignore && isAssocOrComposition(member.type)) {
          // Ensure that target is not @cds.persistence.skip
          if(!isNoDbArtifact(artifact) && isNoDbArtifact(member.target._artifact)) {
            signal(info`"${member.name.absolute}": Removing association "${member.name.element}", target "${member.target._artifact.name.absolute}" is either 'abstract' or annotated with '@cds.persistence.skip'`, member.location);
            member._ignore = true;
          }
        }
      });
