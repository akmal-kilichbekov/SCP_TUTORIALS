...
var { hasErrors } = require('../base/messages');
          ...
          callback :
            (pathDict, env) => {
              env.minimum = env.minimum ||
                (env.location == 'from' ? pathDict.path.some(p => p.where /*filter*/) //!!pathDict.path.filter(p => isEntityOrView(p._artifact))[0].where
                                         : (pathDict.path[0]._navigation && pathDict.path[0]._navigation.kind == 'element' && pathDict.path.length > 1));

              env.full =
                env.full
                //|| ['$projection', '$self'].includes(pathDict.path[0].id)
      ...
      */
      ...
      function cloneOnCondExprTree(expr) {
        ...
        if(fwdAssoc) {
          ...
          return createOnCondition(fwdAssoc, newSrcAlias,  newTgtAlias);
        }
  ...
  */
  function flyTrap(pathDict, env)
  {
    ...
    if(env.location === 'filter' || env.location === 'onCondFrom' || env.location === 'onCondAssoc')
    {
      ...
      if(env.tableAliases && env.tableAliases.includes(head.id) || 
        ['$projection', '$self'].includes(head.id)) {
        path = tail;
      }
