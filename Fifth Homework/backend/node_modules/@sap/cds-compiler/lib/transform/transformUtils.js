...
function getTransformers(model, pathDelimiter) {
  ...
  function addImplicitRedirections(model) {
      ...
      forEachMemberRecursively(artifact, (member, memberName) => {
        ...
        if (member.kind == 'element' && member.target && !member.redirected
          ...
          if (implicitlyRedirected.length == 0 &&
            (hasBoolAnnotation(member.target._artifact, '@cds.autoexpose') ||
            (isComposition(member.type) && !hasBoolAnnotation(member.target._artifact, '@cds.autoexpose', false)))) {
    ...
    // Complain if assoc is requested explicitly by name (not just by '*') or if it's used by a path in the same service
    function autoExcludeAssociation(assoc, assocName, artifact, artifactName) {
      ...
      // Check, if there are xrefs, that have to be transferred, because the assocs are redirected
      if (assoc.origin && assoc.origin._artifact && assoc.origin._artifact._xref) {
        let xrefs = assoc.origin._artifact._xref.filter(xref => xref.user && xref.user._main && xref.user._main._service == artifact._service);
        if (assoc._xref)
          assoc._xref.push(...xrefs);
        else
          setProp(assoc, '_xref', xrefs);
      }
      if (assoc._xref && assoc._xref.length > 0) {
        for (const xref of assoc._xref) {
          if (xref.user) { // && xref.user._main && xref.user._main._service == artifact._service) {
            if (xref.user.name.element)
              signal(error`Association "${artifactName}.${assocName}" is needed in definition of "${xref.user.name.absolute}.${xref.user.name.element}" but target "${assoc.target._artifact.name.absolute}" is not exposed in service "${artifact._service.name.absolute}" by any projection`, xref.where.location);
            else
              signal(error`Association "${artifactName}.${assocName}" is needed in definition of "${xref.user.name.absolute}" but target "${assoc.target._artifact.name.absolute}" is not exposed in service "${artifact._service.name.absolute}" by any projection`, xref.where.location);
            return;
          }
        }
      }
