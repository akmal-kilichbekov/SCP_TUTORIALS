...
const { hasBoolAnnotation } = require('../model/modelUtils');
...
function standard( node ) {
  // Remove nodes that are marked as '_ignore' (TODO: rename to $ignore).
  // TODO: please remove any logic from the XSN-to-CSN transformator !!!
  // * when is a node undefined?
  // * Correct the SQL renderer for virtual non-existing (?!?) elements
  // * Why are artifacts set to _ignore if they should not? (@cds.persistence....?)
  // * Why are contexts declared to be _ignore'd, but not really?
  if (node === undefined ||
      node._ignore === true &&
      !node.virtual &&
      !hasBoolAnnotation(node, '@cds.persistence.skip') &&
      !hasBoolAnnotation(node, '@cds.persistence.exists') &&
      node.kind !== 'context' )
    return undefined;
...
function location( loc, csn, xsn ) {
  if (xsn.kind && xsn.kind.charAt() !== '$' && xsn.kind !== 'query' // XSN TODO: $query
      && !xsn.$inferred
      // no location for inferred elements of query entity (XSN TODO: have proxies there...)
      && (xsn.kind !== 'element' || !xsn._main.query || xsn.name.$mixin)) {
    let l = xsn.name && xsn.name.location || loc;
    // csn.$location
    let value  = { file: l.filename, line: l.start.line, col: l.start.column };
    Object.defineProperty( csn, '$location', { value, configurable: true, writable: true } )
  }
}
