...
const { IllegalFunctionArgumentError, UnexpectedFunctionCallError } = require('../errors')
...
class Where extends BaseStatement {
  ...
  _parseFunctionFromString (aggregation) {
    ...
    if (parts.length === 1) {
      ...
      return { func: parts[0].toLowerCase(), args: [] }
    }

    const args = parts.slice(1, parts.length)
    // special cases like count(*), count(1) or sum(10.1)
    if (parts[1] === '*' || this._isNumber(parts[1])) {
      return { func: parts[0].toLowerCase(), args: [{ val: parts[1] }] }
    }

    return { func: parts[0].toLowerCase(), args: args.map(arg => this._buildRef(arg.trim())) }
  }
  ...
  _isRefExpression (arg) {
    return typeof arg === 'object' && arg.identifier && arg.refname
  }
