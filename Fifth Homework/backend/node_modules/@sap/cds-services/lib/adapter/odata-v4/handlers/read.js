const {
  ...
  uri: {
    UriResource: {
      ResourceKind: { BOUND_FUNCTION, COUNT, FUNCTION_IMPORT, NAVIGATION_TO_ONE }
    }
  }
...
const _getContext = (readContext, shareContext) => {
  if (shareContext) {
    const countContext = Object.assign({}, readContext)

    const atomicityGroupId = countContext._.odataReq.getAtomicityGroupId()

    if (atomicityGroupId) {
      Reflect.setPrototypeOf(countContext, DelayedEmitter.prototype)
      countContext.setStore(countContext._.eventEmitterOfAtomicityGroup.get(atomicityGroupId))
    } else {
      Reflect.setPrototypeOf(countContext, EventEmitter.prototype)
    }
...
const read = (service, options) => {
  return (req, res, next) => {
    ...
    const context = getContextObject(DATA_READ_HANDLER, service, req, res, true)
