...
class Service extends Client {
  ...
  _addPersonalDataHandler (entity) {
    if (
      ...
      this.before('COMMIT', entity, handlers.beforeCommitPersonalData(this._auditLogger))
      this.after('COMMIT', entity, handlers.afterCommitPersonalData)
      this.before('ROLLBACK', entity, handlers.beforeRollbackPersonalData(this._auditLogger))
      this.after('ROLLBACK', entity, handlers.afterRollbackPersonalData)
  ...
  async _destroyConnections (context) {
    if (context._.connections && cds.session) {
      // F.U.B.A.R. we need to destroy the client if one is left
      const destroys = []

      for (const [session, transaction] of context._.connections) {
      ...
      delete context._.connections
      delete context.run
    }
  }

  async _executeFinishTransactionHandlers (command, context, dataOperations) {
    dataOperations = dataOperations || context._.dataOperations || []
  ...
  _addRunIfPrimarySessionExists (context) {
    ...
    context.run = (...args) => {
      if (context._ && context._.streaming) {
        return cds.session.stream(args[0], args[1], context)
      }

      if (this._hasNoSideEffect(context)) {
        return cds.session.run(args[0], args[1], context)
      }

      const tx = cds.session.transaction(context)
      return tx.run(...args)
    }
  }
