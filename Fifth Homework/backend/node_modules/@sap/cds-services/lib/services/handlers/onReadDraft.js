...
const { getEnrichedCQN, removeDraftUUID, ensureDraftsSuffix, ensureNoDraftsSuffix } = require('../utils/draftUtils')
...
const _draftOfWhichIAmOwner = (context, draftWhere, columns) => {
  ...
  const outerMostColumns = _getOuterMostColumns(columns, DRAFT_COLUMNS_CASTED)
...
const _activeWithDraftInProcess = (context, draftWhere, columns, isLocked) => {
  ...
  if (isLocked) {
    subSelect = subSelect.where('InProcessByUser', '!=', { val: context.user.id }, 'and', 'InProcessByUser', '!=', {
      val: null
    })
  } else {
    subSelect = subSelect.where('LastChangedByUser', '!=', { val: context.user.id }, 'and', 'InProcessByUser', '=', {
      val: null
    })
  }
...
const _getUnionCQN = (context, draftName, columns, subSelect, draftWhere) => {
  ...
  draftDocs.columns(...columns, ...DRAFT_COLUMNS_CASTED, 'DraftAdministrativeData_DraftUUID')
...
const _getColumns = ({ query: { SELECT } }, model) => {
  return SELECT.columns
    ...
    : getColumns(model.definitions[ensureNoDraftsSuffix(SELECT.from.ref[0])], true)
}
