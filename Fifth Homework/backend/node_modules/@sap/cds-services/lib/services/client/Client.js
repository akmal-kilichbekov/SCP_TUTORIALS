...
const { FeatureNotSupported, NotInModel } = require('../../errors')
...
class Client {
  ...
  _fillColumnsForSelect (cqn, csnEntity) {
    if (cqn.SELECT && !cqn.SELECT.columns) {
      cqn.columns(getColumns(csnEntity, true))
    }
  }
  ...
  _getCQN (event, entity, columns) {
    switch (event) {
      case 'READ':
        ...
        return this._statements.SELECT.from(entity, Array.isArray(columns) ? columns : undefined, this.model)
      case 'CREATE':
